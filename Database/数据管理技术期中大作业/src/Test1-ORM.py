...
from sqlalchemy import Index, event, create_engine, text
...
# 远程数据库
app.config[
    'SQLALCHEMY_DATABASE_URI'] = f"mysql+pymysql://{id}:{id}@120.46.132.88:3306/db_{id}?charset=utf8mb4"
app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
db = SQLAlchemy(app)
engine = create_engine(f"mysql+pymysql://{id}:{id}@120.46.132.88:3306/db_{id}?charset=utf8mb4")
# print(db)
...
Index('idx_first_name', Employees.first_name)
...
Index('dept_no', Dept_emp.dept_no)
...
class Dept_manager(db.Model):
    ...
    from_date = db.Column(db.Date, nullable=False)
    to_date = db.Column(db.Date, nullable=False)
Index('dept_no', Dept_manager.dept_no)
...
CREATE TRIGGER after_delete_dept_manager
...
rows = []
for row in reader:
    to_date = datetime.strptime(row['to_date'], '%Y-%m-%d') if row['to_date'] else None
    title = Titles(
        emp_no=int(row['emp_no']),
        title=row['title'],
        from_date=datetime.strptime(row['from_date'], '%Y-%m-%d'),
